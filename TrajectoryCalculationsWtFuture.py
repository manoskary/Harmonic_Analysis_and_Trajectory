...
def Trajectory(listofChords, Tonnetz, axes=(0, 0)):
    ...
    for index, chord in enumerate(listofChords):
        if index == 0:
            ...
            newPos = positionOfTheMinNote(
                listofChords[index - 1], chord, ListOfDict[index - 1], Tonnetz)[0]
            axes = newPos
            if not isValidPos(axes):
                axes = positionOfTheMinNote(
                    listofChords[index - 1], listofChords[index + 1], ListOfDict[index - 1], Tonnetz)[0]
                nextChordCoordDict = ChordConfiguration(
                    listofChords[index + 1], axes, Tonnetz)
                newPos = positionOfTheMinNote(
                    listofChords[index + 1], chord, nextChordCoordDict, Tonnetz)[0]
                ListOfDict.append(
                    ChordConfiguration(
                        listofChords[index],
                        newPos,
            ...
            if not isValidPos(axes):
                axes = positionOfTheMinNote(
                    listofChords[index - 2], chord, ListOfDict[index - 2], Tonnetz)[0]
                if not isValidPos(axes):
...
def TrajectoryCheckPosition(
    ...
    if not isValidPos(pos1):
        pos1, lastPos1 = positionOfTheMinNote(
            listofChords[index - 2], ThisChord, listOfDict[index - 2], Tonnetz)
        if not isValidPos(pos1):
            posNext = positionOfTheMinNote(
                listofChords[index - 1], listofChords[index + 1], listOfDict[index - 1], Tonnetz)[0]
            if not isValidPos(posNext):
                raise KeyError('No possible Points')
            else:
                nextChordPoints = ChordConfiguration(
                    listofChords[index + 1], posNext, Tonnetz)
                pos1, lastPos1 = positionOfTheMinNote(
                    listofChords[index + 1], ThisChord, nextChordPoints, Tonnetz)
                if not isValidPos(pos1):
                    raise KeyError('No possible Values')
                else:
                    ThisChordPoints = ChordConfiguration(
                        ThisChord, pos1, Tonnetz)
                    edge = [(pos1, lastPos1)]
                    return ThisChordPoints, edge
        else:
            return TrajectoryLookConnected(
                pos1,
                lastPos1,
                listofChords,
                ThisChord,
...
def TrajectoryLookConnected(
    ...
    if pos1 not in listOfDict[index - 1] or not isValidPos(pos1):
        newpos1, newlastpos1 = positionOfTheMinNote(
            listofChords[index - 2], ThisChord, listOfDict[index - 2], Tonnetz)
...
def TrajectoryCheckSecond(
    ...
    if not isValidPos(pos1):
        ...
        if isValidPos(posNext):
            nextChordPoints = ChordConfiguration(
                listofChords[index + 1], posNext, Tonnetz)
            pos2, lastPos2 = positionOfTheMinNote(
                listofChords[index + 1], ThisChord, nextChordPoints, Tonnetz)
            if isValidPos(pos2):
        ...
        # Do I need to consider taking index-2 here too? (propably not)
...
def TrajectoryFirstAndLastChords(listofChords, listOfDict, index, Tonnetz):
    ...
    pos1, lastPos1 = positionOfTheMinNote(
        listofChords[index - 1], ThisChord, listOfDict[index - 1], Tonnetz)
    if not isValidPos(pos1):
...
def TrajectoryConvexHullComparison(
    ...
    concatPoints1 = concat3DictValues(
        ThisChordPoints1, listOfDict[index - 1], listOfDict[index - 2])
    concatPoints2 = concat3DictValues(
        ThisChordPoints2, listOfDict[index - 1], listOfDict[index - 2])
...
def TrajectoryWithFuture(
    ...
    if index > 1 and index < len(listofChords) - 1:
        ...
        pos1, lastPos1 = positionOfTheMinNote(
            listofChords[index - 1], ThisChord, listOfDict[index - 1], Tonnetz)
        return TrajectoryCheckPosition(
            pos1,
            lastPos1,
            listofChords,
            listOfDict,
        ...
        return TrajectoryFirstAndLastChords(
            listofChords=listofChords,
            listOfDict=listOfDict,
            index=index,
            Tonnetz=Tonnetz)


def TrajectoryLookBeforeLookAhead(
        listofChords,
        listOfBeetweenEdges,
        listOfDict,
        Tonnetz,
...
def NewTrajectory(listofChords, Tonnetz, origin=(0, 0)):
    ...
    for index, chord in enumerate(listofChords):
        if index == 0:
            ...
            newPos, lastPos = positionOfTheMinNote(
                listofChords[index - 1], chord, listOfDict[index - 1], Tonnetz)
            origin = newPos
            if not isValidPos(origin):
                origin = positionOfTheMinNote(
                    listofChords[index - 1], listofChords[index + 1], listOfDict[index - 1], Tonnetz)[0]
                nextChordCoordDict = ChordConfiguration(
                    listofChords[index + 1], origin, Tonnetz)
                newPos, lastPos = positionOfTheMinNote(
                    listofChords[index + 1], chord, nextChordCoordDict, Tonnetz)
            ...
            chordCoord, edge = TrajectoryWithFuture(
                listofChords, listOfBeetweenEdges, listOfDict, Tonnetz, index)
