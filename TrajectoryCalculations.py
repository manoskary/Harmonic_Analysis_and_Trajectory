        ...
        if index == 0:
            ...
            if axes == (104, 104):
                axes = positionOfTheMinNote(
                    listofChords[index - 1], listofChords[index + 1], ListOfDict[index - 1], Tonnetz)
            ...
            if axes == (104, 104):
                axes = positionOfTheMinNote(
                    listofChords[index - 2], chord, ListOfDict[index - 2], Tonnetz)
                if axes == (104, 104):
                    print(chord1, chord2, coordDict1, noteA, noteB)
                    raise RuntimeError("Couldn't match closest points")
            ListOfDict.append(ChordConfiguration(chord, axes, Tonnetz))
    ...
    for index, chord in enumerate(listofChords):
        if index == 0:
            ...
            if axes == newPos:
                ...
                newPosPrev = positionOfTheMinNote(
                    listofChords[index - 2], chord, ListOfDict[index - 2], Tonnetz)
                if newPosPrev == axes:
                    axes = newPosPrev
                else:
