...
def BachTrajectoryGraphs(listOfBachPieces):
...
# Directed Hausdorff on Points (Usually nothing interesting)

	# SVDValues = {}
	# for key in BachTrajectoryPoints :
	#     u, s, vh = np.linalg.svd(BachTrajectoryPoints[key], full_matrices=True)
	#     SVDValues[key] = (u.shape, s.shape, vh.shape)

	# Hausdorff = {}
	# for key1, key2 in itt.product(BachTrajectoryPoints.keys(), BachTrajectoryPoints.keys()):
	#     if key1 != key2 :
	#         array1 = BachTrajectoryPoints[key1]
	#         array2 = BachTrajectoryPoints[key2]
	#         newkey = key1 + 'VS' + key2
	#         Hausdorff[newkey] = (directed_hausdorff(array1, array2)[0])
	return BachGraph




# def BachTrajectoryLookBeforeGraphs(listOfBachPieces):
...
# 	    chordList, Tonnetz = listOfBachPieces[key]
# 	    firstPoint = PlaceFirstNote(chordList, Tonnetz)
# 	    Points, listOfBeetweenEdges = NewTrajectoryLookBefore(chordList, Tonnetz, firstPoint)
# 	    Edges = TrajectoryNoteEdges(Points) + listOfBeetweenEdges
# 	    # for edge in listOfBeetweenEdges:
# 	    # 	[(x, y)] = edge
# 	    # 	if x != y :
# 	    # 		Edges.append(edge)
...
# 	    BachTrajectoryPointWeights[key] = weightsOfTrajPoints(setOfPoints, multiSetOfPoints)
# 	    BachTrajectoryEdges[key] = Edges
# 	    BachGraph[key] = CreateGraph(Points, Edges)
...
# def GraphOfNewPieceLookBefore(newPiece):   
# 	TrajectoryPoints = dict()
# 	TrajectoryPointWeights = dict()
# 	TrajectoryEdges = dict()
# 	Graph = dict()
...
# 		chordList, Tonnetz = analysisFromCorpus(file)
# 	firstPoint = PlaceFirstNote(chordList, Tonnetz)
# 	Points, listOfBeetweenEdges = NewTrajectory(chordList, Tonnetz, firstPoint)
# 	Edges = TrajectoryNoteEdges(Points) + listOfBeetweenEdges
# 	setOfPoints, multiSetOfPoints = SetOfPoints(Points)    
# 	TrajectoryPoints[newPiece] = np.array(setOfPoints)
# 	TrajectoryPointWeights[newPiece] = weightsOfTrajPoints(setOfPoints, multiSetOfPoints)
# 	TrajectoryEdges[newPiece] = Edges
# 	Graph[newPiece] = CreateGraph(Points, Edges)
...
def CentralitiesScatterPlot(dictOfGraphs1, dictOfGraphs2, typePlot='EigenValues'):
	...
	ax.scatter(x1, y1, z1,  alpha=0.6, c='b', edgecolors='none', s=30)
	ax.scatter(x2, y2, z2, alpha=0.8, c='r', edgecolors='none', s=30)
