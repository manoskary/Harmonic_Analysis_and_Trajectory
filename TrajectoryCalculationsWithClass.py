...
def computeChordCoord(thisChord, someChordCoord, Tonnetz) :
    origin, otherRefOrigin = positionOfTheMinNote(list(someChordCoord.keys()), thisChord, someChordCoord, Tonnetz)
...
def TrajectoryConvexHullComparison(placement1, placement2, lastChordCoord, secondLastChordCoord):
    concatPoints1 = concat3DictValues(placement1[0], lastChordCoord, secondLastChordCoord )
    concatPoints2 = concat3DictValues(placement2[0] , lastChordCoord, secondLastChordCoord )
...
def TrajectoryCheckSecond(placement1, trajectory):
    try :
        ...
        placement2 = placeChordWithVirtualRef(trajectory.getThisChord(), lastChordCoord, nextChord, trajectory.Tonnetz)
        return TrajectoryConvexHullComparison(placement1, placement2, lastChordCoord, secondLastChordCoord)
...
def TrajectoryLookConnected(trajectory):
    ...
    thisChordPoints1, edge1 = computeChordCoord(thisChord, trajectory.getLastPosition(), trajectory.Tonnetz)
    if edge1[0][1] != edge1[0][0] :
        try :
            thisChordPoints2, edge2 = computeChordCoord(thisChord, trajectory.getLastPosition(2), trajectory.Tonnetz)
            if edge2[0][1] == edge2[0][0]:
                return TrajectoryCheckSecond((thisChordPoints2, edge2), trajectory)
        except PlacementError :
            pass
...
def TrajectoryCheckPosition(trajectory):
    try :
        ...
        try :
            return computeChordCoord(trajectory.getThisChord(), trajectory.getLastPosition(2), trajectory.Tonnetz)
        except PlacementError :
            numberOfIter = 1
            ...
            while conditionalWhile and numberOfIter < 5:
                try :
                    ...
                    return placeChordWithVirtualRef(trajectory.getThisChord(), trajectory.getLastPosition(), trajectory.getNextChord(numberOfIter), trajectory.Tonnetz)
                except PlacementError :
                    numberOfIter += 1
                    conditionalWhile = True
...
def TrajectoryWithFuture(trajectory):
    if trajectory.index > 1 and trajectory.index < len(trajectory.listOfChords)-1 :
        ...
        return computeChordCoord(trajectory.getThisChord() , trajectory.getLastPosition() , trajectory.Tonnetz)

def placeChordWithVirtualRef(thisPCS, placedChordCoord, tempPCS, Tonnetz):
    virtualChordCoord, _ = computeChordCoord(tempPCS, placedChordCoord, Tonnetz)
